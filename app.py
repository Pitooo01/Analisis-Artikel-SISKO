...
# Cari kata
def cari_kata(teks_list, kata):
    teks = ' '.join(teks_list)
    teks_lower = teks.lower()
    kata_lower = kata.lower()
    jumlah = teks_lower.count(kata_lower)
    return jumlah

# Ganti kata
def ganti_kata(teks_list, kata_lama, kata_baru):
    return [p.replace(kata_lama, kata_baru) for p in teks_list]

# Urut kata
def urutkan_kata(teks_list):
    teks = ' '.join(teks_list)
    import re
    kata_list = re.findall(r'\b[a-zA-ZÀ-ÿ]+\b', teks.lower())
    kata_urut = sorted(set(kata_list))
    return kata_urut

@app.route('/', methods=['GET', 'POST'])
def index():
    selected_fitur = request.args.get('fitur')
    hasil_cari = None
    hasil_ganti = None
    hasil_urut = None
    ...
    if request.method == 'POST':
        if 'cari' in request.form:
            kata = request.form['kata_cari']
            hasil_cari = cari_kata(artikel, kata)
        elif 'ganti' in request.form:
            lama = request.form['kata_lama']
            baru = request.form['kata_baru']
            hasil_ganti = ganti_kata(artikel, lama, baru)
        elif 'urut' in request.form:
            hasil_urut = urutkan_kata(artikel)

    return render_template('index.html', artikel=artikel, selected_fitur=selected_fitur, hasil_cari=hasil_cari, hasil_ganti=hasil_ganti, hasil_urut=hasil_urut)
